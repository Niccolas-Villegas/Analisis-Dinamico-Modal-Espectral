...
class Analisis_Modal:
    def __init__(self, m, k, h):
        ...
        self.MPF , self.EM, self.PM = self.Factor_Part_Modal(self.phi, 
                                                             self.I)
        self.modes, self.results = self.Tablas(self.x, self.T, self.MPF, 
                                               self.EM, self.PM)

    def __str__(self):
        print('Matriz de Masas : \n{}\n'.format(self.M))
        print('Matriz de Rigideces : \n{}\n'.format(self.K))
        return '' 
...
class Analisis_Espectral(Analisis_Modal):
    def __init__(self, m, k, h, S_coeff, R, Tp, Tl):
        ...
        self.Sd, self.D, self.Dr = self.Desplazamientos(self.w, self.Sa, 
                                                        self.MPF, self.phi)
    ...
    def Graficos(self):
        ...
        for index, value in enumerate(self.Vr[::-1]):
            ax4.text(value, index+1, str(np.round(value,2)), 
